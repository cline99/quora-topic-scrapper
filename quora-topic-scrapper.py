...
def extract_ques_ans(html):
    ...
    views_element = soup.find(
        "div",
        class_="q-flex qu-justifyContent--space-between AnswerFooter___StyledFlex-sc-2xbo88-0 kcDxwV",
    )
    views = (
        views_element.get_text(strip=True)[
            0 : views_element.get_text(strip=True).index("view") - 1
        ]
        if views_element
        else None
    )
    shares_element = soup.find_all(
        "div",
        class_="q-click-wrapper qu-display--inline-block qu-tapHighlight--white qu-cursor--pointer qu-hover--textDecoration--underline ClickWrapper___StyledClickWrapperBox-zoqi4f-0 iyYUZT",
    )
    ...
    t_views = views
    t_views = (
        float(views[0 : len(views) - 1]) * 1000
        if views[-1].upper() in ("K")
        else t_views
    )
    t_views = (
        float(views[0 : len(views) - 1]) * 1000000
        if views[-1].upper() in ("M")
        else t_views
    )
    t_views = (
        float(views[0 : len(views) - 1]) * 1000000000
        if views[-1].upper() in ("B")
        else t_views
    )

    if not (
        int(t_views) >= threshold_views and int(number_of_shares) >= threshold_shares
    ):
        pass
    else:
        question_element = soup.find(
            "div",
            class_="QuestionTitle___StyledText-exj38m-0 chNUqN puppeteer_test_question_title",
        )
        question = question_element.get_text(strip=True) if question_element else None
        answer_element = soup.find(
            "div",
...
def topic_scrpper(topic):
    ...
    elements = driver.execute_script(
        'return document.getElementsByClassName("q-text qu-truncateLines--3 qu-wordBreak--break-word");'
    )
